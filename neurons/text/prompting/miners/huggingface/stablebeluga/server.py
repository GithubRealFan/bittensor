...
class StabilityaiProcessor:

    def __init__(self, device):
        self.tokenizer = AutoTokenizer.from_pretrained('stabilityai/StableBeluga-7B', use_fast=False)
        self.model = AutoModelForCausalLM.from_pretrained('stabilityai/StableBeluga-7B', torch_dtype=torch.float16)
        self.pipeline = pipeline(
            "text-generation", self.model, tokenizer=self.tokenizer,
            device=device, max_new_tokens=256, temperature=0.1, do_sample=True, pad_token_id=self.tokenizer.eos_token_id
        )
...
processors = [StabilityaiProcessor(device=i) for i in range(num_gpus)]  # Assuming 8 GPUs
